const NEGATION_RE =
  /(?:안\s*(?:가|해|하|할|갈|먹|만나)|못\s*가|하지\s*(?:마|말)|하지\s*않|않\s*(?:가|해|하)|없(?:음|어|다|습니다)|없어짐|없어졌|없던\s*걸로\s*해|취소(?:됐|됨|해|할|됐어)?|(?:넣|추가|등록)(?:지\s*(?:마|말)|하지\s*(?:마|말))|저장하지\s*(?:마|말)|일정\s*잡(?:지\s*(?:마|말)|지마)|\bno\s+meeting\b|\bnot\s+going\b|cancel(?:led|ed)?)/i;

const RETRIEVAL_QUESTION_RE =
  /(?:\?|뭐지|뭐\s*였|뭐\s*있(?:어|지|나)?|뭐\s*했|몇\s*시(?:였|야|지)|일정\s*(?:뭐|있(?:나|어|지)|보여|알려)|스케줄\s*(?:뭐|보여|알려)|(?:일정|스케줄).*보여\s*줘|맞아|(?:맞지|맞나|기억나|언제(?:더라|였지|였더라))\s*[.!]?\s*$|what\s+do\s+i\s+have|do\s+i\s+have(?:\s+anything)?)/i;

const TENTATIVE_RE =
  /(?:갈까|할까|먹을까|만날까|갈지도|할지도|일지도|것\s*같|수도\s*있|잡을\s*듯|할\s*듯|갈\s*듯|아마|maybe|might|could|possibly|probably)/i;

const EDIT_COMMAND_RE =
  /(?:\b아니\b|아니\s+.*말고|\d{1,2}\s*시\s*말고|(?:내일|오늘|모레)\s*말고|(?:^|\s)말고(?=\s|$)|(?:바꿔|바꾸|변경(?:해|하기|으로)?|일정\s*삭제|일정\s*취소|그거\s*(?:취소|삭제)|아까\s*일정)|(?:^|\s)삭제(?:해|해줘|할래|했어)?(?=\s|$)|(?:오전|오후)\s*\d{1,2}\s*시(?:\s*반|\s*\d{1,2}\s*분)?\s*로\s*$|^(?:오전|오후)\s*(?:이야|야)\s*$)/i;
...
const PAST_WORD_RE =
  /(?:어제|그제|지난\s*주|지난주|저번\s*주|저번주|지난\s*달|지난달|저번\s*달|저번달|(?:지난|저번|지난번)\s*(?:일|월|화|수|목|금|토)요일|작년|yesterday|last\s+week|last\s+month|last\s+year)/i;
