...
const EDIT_COMMAND_RE =
  /(?:\b아니\b|아니\s+.*말고|\d{1,2}\s*시\s*말고|(?:내일|오늘|모레)\s*말고|(?:바꿔|바꾸|변경(?:해|하기|으로)?|일정\s*삭제|일정\s*취소|그거\s*(?:취소|삭제)|아까\s*일정)|(?:^|\s)삭제(?:해|해줘|할래|했어)?(?=\s|$)|(?:오전|오후)\s*\d{1,2}\s*시(?:\s*반|\s*\d{1,2}\s*분)?\s*로\s*$|^(?:오전|오후)\s*(?:이야|야)\s*$)/i;
