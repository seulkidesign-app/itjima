const NEGATION_RE =
  /(?:안\s*(?:가|해|하|할|갈|먹|만나)|못\s*가|하지\s*말|하지\s*않|않\s*(?:가|해|하)|없(?:음|어|다|습니다)|없어짐|없어졌|없던\s*걸로\s*해|취소(?:됐|됨|해|할|됐어)?|(?:넣|추가|등록)(?:지\s*(?:마|말)|하지\s*(?:마|말))|저장하지\s*(?:마|말)|일정\s*잡(?:지\s*(?:마|말)|지마)|\bno\s+meeting\b|\bnot\s+going\b|cancel(?:led|ed)?)/i;
