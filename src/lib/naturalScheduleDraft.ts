...
export function cleanScheduleTitle(text: string): string {
  ...
  title = title
    ...
    // Strip a from–to range as one semantic time expression before single-clock cleanup.
    .replace(/(?:(?:오전|오후)\s*)?\d{1,2}\s*시(?:\s*반|(?:\s*\d{1,2}\s*분))?\s*부터\s*(?:(?:오전|오후)\s*)?\d{1,2}\s*시(?:\s*반|(?:\s*\d{1,2}\s*분))?\s*까지/g, " ")
    ...
    // Korean clock cleanup can leave the case particle behind ("5시에 청소" -> "에 청소").
