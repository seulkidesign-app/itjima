...
export function ArchiveRelatedStrip({ item, items, onSelect }: Props) {
  ...
  return (
    <div className="mt-3 rounded-[16px] bg-ink/[0.03] px-3 py-2.5">
      ...
      <div className="mt-2 flex flex-col gap-1">
        {related.map(({ item: rel }) => (
          ...
          >
            <span className="block line-clamp-1 text-[13px] font-semibold text-ink">
              {archiveDisplayTitle(rel.id, rel)}
            </span>
            <span className="mt-0.5 block line-clamp-1 text-[11px] text-ink-soft/75">
