...
export function ScheduleSheet({
  ...
  const end0 = initialEnd ?? (() => { const d = new Date(now); d.setHours(d.getHours() + 1); return d; })();
  const [text, setText] = useState(initialText);
  const [start, setStart] = useState([now.getMonth() + 1, now.getDate(), now.getHours(), now.getMinutes()]);
  const [end, setEnd] = useState([end0.getMonth() + 1, end0.getDate(), end0.getHours(), end0.getMinutes()]);
  ...
  const colDef = [
    ...
    { label: t("분", "Min"), values: [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55], pad: 2 },
  ];
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col" onClick={onClose}>
      <div className="flex-1 transition-opacity duration-300" style={{background:"rgba(0,0,0,0.5)"}} />
      <div
        className="animate-slide-up rounded-t-[28px] bg-background px-5 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] pt-2.5 shadow-[0_-12px_40px_-12px_rgba(0,0,0,0.25)]"
