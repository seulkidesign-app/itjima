...
import {
  authDebugExchangeCodeForSession,
  authDebugSignOut,
} from "@/lib/authDebug";
...
function createSupabaseClient() {
  ...
  if (typeof window !== "undefined" && import.meta.env.VITE_E2E !== "true") {
    const auth = client.auth as typeof client.auth & {
      exchangeCodeForSession: typeof client.auth.exchangeCodeForSession;
    ...
    auth.exchangeCodeForSession = async (code: string) => {
      authDebugExchangeCodeForSession("supabase.auth.exchangeCodeForSession", code);
      return originalExchange(code);
    };
    const originalSignOut = auth.signOut.bind(auth);
    auth.signOut = async (...args) => {
      authDebugSignOut("supabase.auth.signOut");
